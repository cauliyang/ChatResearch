...
class Reader:
    ...
    def get_arxiv_web(self, args, page_num=1, days=2):
        titles, links, dates = self.get_all_titles_from_web(
...
def add_subcommnd(parser):
    ...
    subparser.add_argument(
        "--page-num", type=int, default=1, help="the maximum number of page"
    )
    subparser.add_argument(
    ...
    subparser.add_argument(
        "--sort", type=str, default="web", help="another is LastUpdatedDate"
    )
    subparser.add_argument(
        "--save-image",
        default=False,
